...
*/
    ...
    module.Orderline = Backbone.Model.extend({
        ...
        set_discount: function(discount){
            var disc = Math.round(Math.min(Math.max(parseFloat(discount) || 0, 0),100));
            this.discount = disc;
        ...
        export_as_JSON: function() {
            ...
            var total = result['qty'] * result['price_unit'];
            var currentOrder = this.pos.get('selectedOrder');
            var member_discount = 1;
            if(currentOrder.get_member()){
                member_discount = currentOrder.get_member().discount
            }else{
                member_discount = 1;
            }
            result['price_subtotal_incl'] = total * member_discount;
            return result;
        },
        ...
        get_price_with_tax_no_dis: function(){
            return this.get_all_prices(true).priceWithTax;
        },
        ...
        get_all_prices: function(no_dis){
            var no_dis = no_dis || false;
            var self = this;
            ...
            if(no_dis) {
                base = round_pr(this.get_quantity() * this.get_unit_price(), currency_rounding);
            }
            var totalTax = base;
            var totalNoTax = base;
    ...
    module.Order = Backbone.Model.extend({
        ...
        validate_dead_time_sms: function(enter_verify_seconds){
            var valid = false;
            var sended_seconds = (new Date() - this.get('member').send_verfy_time)/1000;
            if(sended_seconds > enter_verify_seconds){
                valid = true;
            }
            return valid
        },

        get_paymentline_amount: function(type_code){
            var amount = 0;
            this.get('paymentLines').each(function(line){
                if(line.cashregister.journal.code.toLowerCase() == type_code.toLowerCase()){
        ...
        //设置发送短信验证码的时间
        set_send_verify_time:function() {
            this.get('member').send_verfy_time = new Date();
        },


        //设置当前的短信验证码
        set_curr_verify: function(verify_code) {
            this.get('member').verify_code = verify_code;
        },

        //获取当前的短信验证码
        get_curr_verify: function() {
            return this.get('member').verify_code;
        },
        ...
        get_point_by_code: function(code){
            var point = 0;
            var rule = this.get('member').points_rule;
            this.get('paymentLines').each(function(line){
                if(line.cashregister.journal.code.toLowerCase() == code.toLowerCase()){
                    point = line.get_amount() * rule;
        ...
        set_total_point:function(total_point){
            this.set('total_point',total_point);
        },

        get_all_discMoney: function(){
            var discMoney = 0;
            this.get('orderLines').each(function(line){
                discMoney = discMoney + line.get_display_price();
            });
            var rounding = this.pos.currency.rounding;
        ...
        addPaymentline: function(cashregister) {
            ...
            //当已经存在相同的付款方式时，先移除，再重新添加一个
            for (var i = 0; i < paymentLines.models.length; i++) {
                if(cashregister.journal.code.toLowerCase() == paymentLines.models[i].cashregister.journal.code.toLowerCase()){
                    this.removePaymentline(paymentLines.models[i]);
                    break;
                }
            };
        ...
        getTotalTaxIncludedNoDis: function() {
            return (this.get('orderLines')).reduce((function(sum, orderLine) {
                return sum + orderLine.get_price_with_tax_no_dis();
            }), 0);  
        },
